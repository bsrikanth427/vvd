...
public class AdminUploadService {
    ...
    //    @Autowired
    //    private MediaTagsRepository mediaTagsRepository;
    ...
    public Page<AdminMediaEntity> getNewAdminUploadByDate(Pageable pageable) {
        Page<AdminMediaEntity> mediaEntities = adminUploadFormRepository.findByOrderByCreatedDateDesc(pageable);
        return mediaEntities;
    }

    public Page<AdminMediaEntity> getOrderByPopular(Pageable pagingSort) {
        Page<AdminMediaEntity> mediaEntities = adminUploadFormRepository.findByOrderByTotalOrdersDesc(pagingSort);
        return mediaEntities;
    }

    public Page<AdminMediaEntity> search(Pageable pageable, String searchKey) {
        List<AdminMediaEntity> mediaEntities = adminUploadFormRepository.search(searchKey);
        List<AdminMediaEntity> categoryList = adminUploadFormRepository.findByNameIgnoreCaseContaining(searchKey);
        Page<AdminMediaEntity> page = null;
        if (!categoryList.isEmpty() || null != categoryList) {
            categoryList.forEach(
                media -> {
                    if (media.getName().equalsIgnoreCase(searchKey)) {
        ...
        if (!mediaEntities.isEmpty() && null != mediaEntities) {
            final int end = Math.min((start + pageable.getPageSize()), mediaEntities.size());
            page = new PageImpl<>(mediaEntities.subList(start, end), pageable, mediaEntities.size());
            return page;
        }
        return page;
    }
